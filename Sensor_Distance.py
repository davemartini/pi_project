...
def distanceMath(start, end): #distance math function 
    duration = end-start
...
def pollPosition():
    ...
    while True:
        ...
        while GPIO.input(ECHO) == 0: #echo is the input pin on the sensor, after pulse is sent, the sensor reads a zero and sets time to start then when input is read it is set to time end
            start = time.time()
...
def checkThresh(currentDistance):
    threshDeviation = thresh+5 #adds five cm to the threshold measurement to decrease the likelihood of a false positive alarm

    if(currentDistance > threshDeviation):
        alarm()

def alarm(): #alarm function triggers the pin to send up signal to buzzer, setting it off and leaving it on until manually interrupted by keyboard
    while True:
...
try:
    while True: #main while loop that loops awaiting input from the button to bring the signal to the down position to trigger the alarm process
        untriggered = GPIO.input(23) #untriggered initilizes to 1 as was set at the begining of the script setup
        if(untriggered == 0):
            armed()
            setThresh()
            pollPosition()
